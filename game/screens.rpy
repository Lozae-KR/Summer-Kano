...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("이전으로") action Rollback()
            ...
            textbutton _("빠른로드") action QuickLoad()
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("출석 {size=-10}(게임시작){/size}") action Start()

        else:

            textbutton _("필기노트 {size=-10}(대화이력){/size}") action ShowMenu("history")

            textbutton _("필기하기 {size=-10}(저장하기){/size}") action ShowMenu("save")

        textbutton _("복습하기 {size=-10}(불러오기){/size}") action ShowMenu("load")

        textbutton _("스펙확인 {size=-10}(업적및 엔딩){/size}") action ShowMenu("achievements_screen")


        if _in_replay:

            textbutton _("복습 끝 (리플레이종료)") action EndReplay(confirm=True)

        elif not main_menu:

            textbutton _("자체공강 {size=-10}(메인메뉴){/size}") action MainMenu()

        if renpy.variant("pc"):

            ## iOS에서는 종료 버튼이 금지되어 있으며 Android 및 웹에서는 불필요
            ## 합니다.
            textbutton _("자체휴강 {size=-10}(게임종료){/size}") action Quit(confirm=not main_menu)

        if renpy.variant("pc") or (renpy.variant("web") and not renpy.variant("mobile")):

            ## 도움말 메뉴는 모바일 디바이스와 맞지 않아 불필요합니다.
            textbutton _("전공책 {size=-10}(조작방법){/size}") action ShowMenu("help")

        textbutton _("족보 {size=-10}(게임정보){/size}") action ShowMenu("about")
...
screen main_menu():
    ...
    if gui.show_name:
        ...
        vbox:
            ...
            text "전체화면{size=-8} (F11){/size} 적용 권장드립니다.\n{size=-10}Ver. [config.version]{/size}":
...
screen about():
    ...
    ## 다시 불려집니다.
    use game_menu(_("게임정보"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("{a=https://www.renpy.org/}Ren'Py (렌파이){/a} [renpy.version_only] 버전으로 제작되었습니다.\n\n[renpy.license!t]")
...
screen history():
    ...
    use game_menu(_("필기노트"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0, spacing=gui.history_spacing):
        ...
        if not _history_list:
            label _("배운게 없는 듯 하다 아니... 필기를 안한건가?")
...
screen help():
    ...
    use game_menu(_("전공책"), scroll="viewport"):
...
screen achievements_screen():
    ...
    use game_menu(_("스펙 확인"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                spacing 20
                textbutton _("업적") action SetScreenVariable("duqn", "ekftjd")
                textbutton _("갤러리") action SetScreenVariable("duqn", "gallery_screen")

            if duqn == "ekftjd":
                use ekftjddjqwjr
            elif duqn == "gallery_screen":
                use gallery_screen
        ...
        vbox:
            ...
            for ending_id, data in endings.items():
                if data["completed"]:
                    hbox:
                        text "[data['name']]"
                        textbutton "보기" action Show("ending_detail", ending_id=ending_id)
                        
# 업적 상세 화면
screen ending_detail(ending_id):
    tag menu
    frame:
        style_group "menu"
        xalign 0.5
        ...
        vbox:
            text "[endings[ending_id]['name']]" size 30
            text "[endings[ending_id]['description']]" size 20
            add endings[ending_id]['image']  # 업적 이미지 추가
            
            textbutton "뒤로가기" action Return()
