...
label start:
    ...
    # 초기 애정도 설정 (사용자 코드 참고)
    default affection_i = random.randint(20,80)
    if affection_i >=51:
...
label scene_2_lecture_morning:
    ...
    na "???"
    na "(누... 누구지? 인데? 저 얼굴... 어디서 본 것 같기도 하고... 아닌 것 같기도 하고... 근데 왜 이렇게 싱그러워? 여름 햇살 같네...)"
...
label choice_1_result:
    ...
    menu:
        ...
        "...": # "(아무 말 없이 그녀를 쳐다본다)" 선택 시
            # "(아무 말 없이 그녀를 쳐다본다)" 선택 시 반응
    ...
    show a_in at Pright # 인하 우물쭈물 표정
    i "근데... 너 이름이 뭐였지...?"
...
label scene_3_lecture_class:
    ...
    menu:
        ...
        "원래 같이 하기로 한 형이랑 같이 한다":
            ...
            $ affection_i -= random.randint(1, 5) # 애정도 하락 (사용자 코드)
...
label choice_2_decline: # "원래 같이 하기로 한 형이랑 같이 한다" 선택 시
    ...
    if affection_i >= 41: # 애정도 높을 때 거절 -> 찜찜함 + 타임슬립 복선
        ...
        m "별다른 일 없이 하루가 지나가는구나."
        m "씻고 좀 쉬어야겠다."
        cn "(평범한 저녁 시간을 보낸다. 매미 소리가 밤까지 들리는 것 같다.)" # 여름 느낌
...
label choice_4_cafe: # "카페에서 먹자" 선택 시
    ...
    i "음~ 여기 커피 맛있다! [player_name]은 뭐 마셨어?"
    m "나는... 아이스 아메리카노."
...
label choice_4_restaurant: # "밥 먹으면서 얘기하자" 선택 시
    i "엇! 좋아! 근처에 맛있는 파스타 집 있는데 거기 갈래? 에어컨도 빵빵하고!" # 여름 느낌
    m "콜!"
    cn "(파스타 집으로 향한다. 시원한 내부 공기에 기분이 좋아진다.)" # 여름 느낌
    # TODO: 파스타집 배경 이미지 필요
    ...
    m "(와... 분위기 좋다... 데이트 느낌 제대로 나는데? 😳 이 더위에 이런 곳이라니...)" # 여름 느낌 + 순애 느낌

    show s_in # 인하 중앙으로 (대화 포지션) - 필요 시 위치 조정

    i "여기 파스타 진짜 맛있어! 특히 로제 파스타 강추!"
    m "오! 그래? 그럼 나도 로제 파스타 시켜봐야겠다!"

    i "좋아 좋아! ㅋㅋㅋ 아, 그리고 우리 조별 과제 얘기도 해야지! 주제 뭐로 정할지 생각해봤어?"

    cn "(인하가 들고 온 노트를 꺼낸다.)" # 지문

    i "나는 뭔가 재미있는 거 해보고 싶은데!"
